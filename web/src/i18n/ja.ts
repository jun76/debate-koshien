...
export const ja = {
  ...
  wizard: {
    ...
    teamBOption: "チームB",
    coinToss: "コイントス",
    teamHeading: (key: string) => `チーム ${key}`,
    teamName: "チーム名",
    teamMode: "運用方式",
    ...
    reviewerNote: "審査員 1 が感想戦の解説（コメンテーター）を兼任します",
    ...
    exhibitionLabel: "エキシビションモード（試合を丸ごと先に生成してから、待ち時間なしで再生）",
    exhibitionTitle: "全ての推論と音声生成を先に済ませてから、待ち時間なしで1試合を通しで観戦するモード",
    createStart: "試合を作成して開始",
    exhibitionCreateStart: "エキシビション試合を生成して開始",
    creating: "作成中…",
    exhibitionGenerating: (progress: string) => `エキシビション生成中: ${progress}`,
    exhibitionNote: "開戦から決着までの推論と音声を先に生成しています。完了すると自動で観戦が始まります。",
    generatingMatch: "試合を生成中…",
    exhibitionInterrupted: "エキシビションの生成が中断されました",
    mockConfirmTitle: "全員 Mock プロバイダです",
    mockConfirmMessage:
      "Mock プロバイダでは推論は行われず、動作確認用の定型文が生成されます。\n実際に議論させるには Claude Code / Codex / OpenCode 等の CLI を予めセットアップの上、メンバー・審査員のプロバイダとして選択してください。",
    mockConfirmProceed: "Mock で続行",
    configFileError: (msg: string) => `設定ファイルを読み込めません: ${msg}`,
    notJsonObject: "JSON オブジェクトではありません",
    advanced: "詳細設定（名前・モデル・推論）",
